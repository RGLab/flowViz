...
time-consuming, and in many cases makes sense to explicitely create
...
the purose of this demonstration it is sufficient to know that the
...
around with various panel arangements. However, we still pass the
...
dimensions. Imaging you want to make slight changes to your plot (and
...
settings in in the \Robject{scale} argument.
...
Deepayan Sarkar's excellent book \citet{Sarkar} for details. With
respect to \Rpackage{flowViz}, it should suffice to mention that
customization follows the exact same principals defined in the
\Rpackage{lattice} package. As a matter of fact, most customization
actually happens at the level of the underlaying \Rpackage{lattice}
software. The facts to mention are:

\begin{itemize}
\item There are session-wide global defaults that can be queried and
  set using the \Rfunction{flowViz.get.par} and
  \Rfunction{flowViz.set.par} functions. These are extensions to the
  \Rfunction{trellis.get.par} and \Rfunction{trellis.set.par}
...
\item Parameters are split up into logical categories and have to be
  ...
  relevant for filter plotting are \Robject{gate} and
  \Robject{gate.text}, the basic \Rpackage{flowViz} settings are
  controlled by the \Robject{flow.symbol} category. See the
  documentation for \Rfunction{flowViz.par.set} for a complete list of
  available parameters. All additional graphical parameters (like
  \Robject{cex}, \Robject{pch} and \Robject{col}) known from the
  \Rpackage{lattice} high-level plotting functions are also still
  available.
...
We could have archived the same customization by changing the session defaults:
...
than using opque colors, lots of the underlying structure could still
...
components can themselve be valid R expressions including fuction
...
the margins of the data range. This is most notably in the
...
\Rpackage{lattice} encourages the use of expressions in the formula
...
however the tool has to be used with great care, particularily when
plotting gates.
