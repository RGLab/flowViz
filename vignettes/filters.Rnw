...
The \Rpackage{flowCore} package defines the concepts of
\Rclass{filters} and \Rclass{filterResults}.  Filters are abstract
...
result of applying a filter to one or more \Rclass{flowFrames} (data
...
filters or gates inherit from class \Rclass{filter}, whereas objects
generated as the result of applying a filter inherit from class
\Rclass{filterResult}.


\subsection{Visualization}
Before we discuss how to visualize gates in \Rpackage{flowViz}, we need
...
e.g. \Rclass{norm2Filter} gates, which have a (data dependent)
...
Visualizing \Rclass{filterResults} is more general.  Specifically, the
...
the \Rclass{filterResult} once and pass it on to the plotting
functions instead of the input filter. In the course of this manual,
\Rclass{filter} objects and \Rclass{filterResults} may be used
interchangeably, unless stated otherwise.



\subsection{Example Data}
We use the \Rdata{GvHD} data set to provide some examples. It come as
a serialized \Rclass{flowSet} with the \Rpackage{flowViz} package and
...
\Robject{phenoData} slot of the \Robject{GvHD} set contains several
factor variables, \Robject{Patient} and \Robject{Visit} two of the
most descriptive ones. In general, all \Rclass{flowSets} contain an
implicit \Robject{name} variable which is is used as the default
conditioning variable in all of \Rpackage{flowViz's} high-level
plotting functions.
<<loadData>>=
library(flowViz)
data(GvHD)
...
The \Rpackage{lattice} package offers inline subsetting capabilities
...
\Rpackage{lattice} concepts) is also available for all derived
plotting methods in \Rpackage{flowCore}. The general idea here is that
all symbols defined in either the formula or one of the special
arguments like \Robject{subset} are evaluated in the context of the
\Rclass{flowFrame's} \Robject{phenoData} data frame or the raw data
matrix. This is a slight extension of the fundamental
\Rpackage{lattice} idea necessitated by the fact that raw data and
annotation data are stored separately in a \Rpackage{flowSet}.
...
whole \Robject{GvHD} object down to the plotting function, potentially
...
adequate log-like scale. A good choice is the \Rfunction{asinh}
...
\Rfunction{transformList} and \Rfunction{\%on\%} functions in the
...
\item \Rclass{filters} and \Rclass{filterResults} can be used
  interchangeably. Providing \Rcode{filterResults} directly may
...
\Rclass{filterResult}. All parameters (e.g. min and max values in a
recangular gate) are unambigously defined in the \Rclass{filter}
object. We will start with a simple \Rclass{rectangleGate} in the
...
\begin{figure}[htb]
  \centering
  \includegraphics{./simpleRG}
  \label{fig:simpleRG}
  \caption{A simple \Rclass{rectangleGate} visualization in the
    \Robject{FSC-H}-\Robject{SSC-H} xyplo.}
\end{figure}
...
arguments when defining the \Rclass{filter}. The software also decides
...
@ 
%
Most of the customization options defined in the \Rpackage{lattice}
...
individual points), as shown in Figure~\ref{fig:.nonSmoothScatter}.
%
%
<<nonSmoothScatter>>=
...
@
  \includegraphics{./plotnonSmoothScatter}
  \label{fig:nonSmoothScatter}
  \caption{Filter visualization in a non-smooth xyplot.}
\end{figure}
%
Additionally, We can add statistics of gated population along with the \Rclass{filter} to the plot
using the \Robject{stat} argument.\Robject{pos} and \Robject{abs}(whether the position is absolute or relative to filter boundary ) are used to control the label position.
%
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotnonSmoothScatterOutline}
  \label{fig:nonSmoothScatterOutline}
  \caption{Rectangle gate with outline in the  the
    \Rclass{FSC-H}-\Rclass{FL4-H} non-smooth xyplot.}
\end{figure}
%
...
arbitrarily complex \Rclass{filters} using different colors or point
...
\Robject{FSC-H} and \Robject{SSC-H} dimensions. The result is shown in 
Figure~\ref{fig:nonSmoothScatter2}
...
print(plot(trellis.last.object()))
@
\includegraphics{./plotnonSmoothScatter2}
  \label{fig:nonSmoothScatter2}
  \caption{Three-dimensional rectange gate with outline in the
    \Rclass{FSC-H}-\Rclass{FL4-H} non-smooth xyplot.} 
\end{figure}
...
(\Rclass{kmeansFilter}, \Rclass{timeFilter}), although one might be
able to fall back to approximations like convex hulls or similar. So
far, \Rpackage{flowViz} has nor support for plotting gate outlines of
these filter types, and they are silently ignored when passed on as
argument \Robject{filter} to any of the high-level plotting
...
individual events is available for virtually all \Rclass{filter}
types.
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotnorm2Filter}
  \label{fig:norm2Filter}
  \caption{Gate defined by \Robject{norm2Filter} in the \Rclass{FSC-H} and
    \Rclass{FL4-H} channels.}
\end{figure}


There are data-driven filter types that may result in more than just
one sub-population. \Rclass{curv1Filters} and \Rclass{curv2Filters}
are a prominent example. In the next code chunk we use a
\Rclass{curv2Filter} to identify high-density areas in the
\Robject{FSC-H} and \Robject{FL4-H} projections of the data.
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotcurv2Filter}
  \label{fig:norm2Filter}
  \caption{High density areas identified by \Robject{curv2Filter} in the
    \Rclass{FSC-H} and \Rclass{FL4-H} dimensions.}
\end{figure}



This is a good example for a case where passing the
\Rclass{filterResult} instead of the \Rclass{filter} helps to optimize
...
approach); having to recompute the \Rpackage{filterResult} on each
iteration would be very time-cosuming and annoying at best.

\section{Filters in one-dimensional density plots}
The \Rpackage{lattice} package provides the high-level
\Rfunction{densityplot} function to draw one-dimensional Kernel
...
\Rfunction{densityplot} methods in the \Rpackage{flowViz} package
...
display impratical. See the documention of \Rfunction{densityplot} for
more details.
...
individual events any more. Similar to the \Rfunction{xyplot} methods,
we can pass \Rclass{filters} or \Rclass{filterResults} to the function
as optional argument \Robject{filter}. We decided to indicate gate
...
print(plot(trellis.last.object()))
@
  \includegraphics{plotdensityplots}
  \label{fig:densityplots}
  \caption{High density areas identified by \Rclass{curv1Filter}.}
\end{figure}
...
familiar objects of class \Rclass{filter}, \Rclass{filterResult},
\Rclass{filterResultList}, or \Robject{NULL} in case no filter should
...
print(plot(trellis.last.object()))
@
    \includegraphics{plotmultDensityplots}
  \label{fig:multDensityplots}
  \caption{Multiple densityplots with combination of \Rclass{rectangleGate} and
    \Rclass{curv2Filter}.}
\end{figure}
...
\Rpackage{lattice} package and can be reversed using appropriate
settings in in the \Robject{scales} argument.

\section{Plotting parameters}
\Rpackage{Lattice} graphics are a great tool to easily create
...
\citet{Sarkar} for details. With
respect to \Rpackage{flowViz}, it should suffice to mention that
customization follows the exact same principles defined in the
\Rpackage{lattice} package. As a matter of fact, most customization
actually happens at the level of the underlying \Rpackage{lattice}
software. The facts to mention are:

\begin{itemize}
\item There are session-wide global defaults that can be queried and
  set using the \Rfunction{flowViz.par.get} and
  \Rfunction{flowViz.par.set} functions. These are extensions to the
  \Rfunction{trellis.par.get} and \Rfunction{trellis.par.set}
  functions in the \Rpackage{lattice} package, and work in exactly the
  same way. All graphical parameters that are not defined in
  \Rpackage{flowViz} are directly passed on to \Rpackage{lattice}.
  
\item Parameters can also be set for a single function call using the
  \Robject{par.settings} argument. These setting take precedence over
  global settings.
  
\item Parameters are split up into logical categories and have to be
  provided as named lists or lists of lists. The categories directly
  relevant for filter plotting are \Robject{gate},
  \Robject{gate.density}, and \Robject{gate.text}, the basic
  \Rpackage{flowViz} settings are controlled by the
  \Robject{flow.symbol} category. See the documentation for
  \Rfunction{flowViz.par.set} for a complete list of available
  parameters. All additional graphical parameters (like \Robject{cex},
  \Robject{pch} and \Robject{col}) known from the \Rpackage{lattice}
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotPars1}
  \label{fig:Pars1}
  \caption{Transparent rectangate gate (\Rclass{rectangleGate}) in the 
     \Robject{FSC-H}-\Robject{FL4-H} xyplot as using the \Robject{per.ettings}
     argument.} 
\end{figure}
...
different effects. The \Robject{col} parameters for instance sets the
line color for filter boundaries and the point color for non-smooth
scatter plots (and also the outline if \Robject{outline=TRUE}). In the
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotPars3}
  \label{fig:Pars3}
  \caption{Rectangate gate
    (\Rclass{rectangleGate}) in the  \Robject{FSC-H}-\Robject{FL4-H}
    non-smooth xyplot as using the \Robject{per.ettings} argument. }
\end{figure}
...
we can spice up out plot of the \Rclass{curv2Filter} results by using
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotPars4}
  \label{fig:Pars4}
  \caption{High density areas in the \Robject{FSC-H}-\Robject{FL4-H}
    xyplot using lists of graphical paremeters.} 
\end{figure}
...
plot. \Rpackage{flowCore} provides default names for each
\Rclass{filter} and their associated results, but the user is free to
pass their own custom names. The interface is very simple: argument
\Robject{names} either takes a logical scalar (default names are used
if \Robject{TRUE}) or a character vector. Again, recycing rules apply
...
print(plot(trellis.last.object()))
@
  \includegraphics{plotnorm2FilterNames}
  \label{fig:norm2FilterNames}
  \caption{Displaying population names.}
\end{figure}
...
\Rpackage{flowViz} are evaluated in the context of either the raw data
matrix or the \Robject{phenoData} slot. This implies that formula
...
\Rpackage{flowViz} potentially changes the scale of the raw data, in
...
In summary, \Rpackage{lattice} encourages the use of expressions in the formula
interface and for the sake of customizability we didn't want to
exclude this feature completely in the \Rpackage{flowViz} methods.
...
print(plot(trellis.last.object()))
@
  \includegraphics{./plotparCoord}
  \label{fig:parCoord}
  \caption{Parallel coordinates plot all channels excluding time.}
\end{figure}
